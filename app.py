...
st.title("🥇 GAPS ETF 대시보드 [V27 - 고정기간 쾌적화 모델]")
st.markdown("💡 연산 속도 최적화를 위해 회귀 모델의 학습 데이터 기간을 **500일, 700일, 1000일**로 고정하여 선택할 수 있습니다.")

# --- 좌측 사이드바 설정 ---
st.sidebar.header("⚙️ 앙상블 모델 설정")
train_window_option = st.sidebar.selectbox(
    "학습 데이터 기간 (가중치 산출용)",
    ["1000 영업일 (약 4년)", "700 영업일 (약 2.8년)", "500 영업일 (약 2년)"]
)
...
@st.cache_data(ttl=21600, show_spinner="⏳ 선택한 고정 기간으로 앙상블 회귀 분석 중... (약 10~15초 소요)")
def run_full_analysis(df_raw, train_window_option):
    ...
    # 1000 영업일 처리를 위해 데이터를 넉넉하게 5년치(약 1250일)만 불러옵니다. 속도 최적화.
    end_date = datetime.today().strftime('%Y-%m-%d')
    start_date = (datetime.today() - timedelta(days=365*5)).strftime('%Y-%m-%d')

    # 사이드바 옵션 텍스트에서 숫자를 추출
    if "1000" in train_window_option:
        fit_window = 1000
    elif "700" in train_window_option:
        fit_window = 700
    else:
        fit_window = 500
...
if os.path.exists(csv_filename):
    ...
    if df_raw is not None:
        ...
        st.sidebar.success(f"📂 V27 엔진 로드 완료 ({train_window_option} 적용)")
        ...
        with tab4:
            ...
            c3.metric("반영 기간", train_window_option.split(' ')[0])
